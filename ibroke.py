...
__version__ = "0.2.0"
__all__ = ('IBroke', 'Instrument', 'Order', 'Quote', 'Bar', 'Tick')
...
class IBroke:
    ...
    def __init__(self, host='localhost', port=7497, client_id=None, timeout_sec=5, verbose=3):
        ...
        self._quote_handlers = defaultdict(list)    # Maps (quote_type, quote_size, instrument_id) to list of functions to be called with those quote events
        ...
        start = time.time()
        while not self.connected:           # None intially meaning never connected; set False by _error(); set True by _handle_message()
    ...
    def register(self, instrument, on_bar=None, on_order=None, on_quote=None, on_alert=None, on_tick=None, bar_type='time', bar_size=1, quote_type='time', quote_size=1):
        ...
        :param func on_quote: Call ``func(instrument, quote)`` with a quote tuple every `quote_size` seconds.
        :param func on_tick: Call ``func(instrument, tick)`` with a tick tuple for each tick update of `instrument`.
        ...
        :param float bar_size: The period of a bar in seconds.
        :param str quote_type: The type of quote to generate, currently only supports `'time'`.
        :param float quote_size: The period in seconds at which to generate quotes.
        """
        assert bar_type in ('time',)
        assert bar_size > 0
        assert quote_type in ('time',)
        assert quote_size > 0
        assert all(func is None or callable(func) for func in (on_bar, on_tick, on_quote, on_order, on_alert))
        instrument = self.get_instrument(instrument)
        if on_tick or on_bar or on_quote:
            # We need to accumulate ticks to make bars/quotes out of.
            ...
            if on_bar:
                ...
                RecurringTask(lambda: self._call_bar_handlers(bar_type, bar_size, instrument.id), interval_sec=bar_size, init_sec=1, daemon=True)        # I guess this keeps going even without a reference...
            if on_quote:
                if len(frozenset(inst.id for _, _, inst in self._quote_handlers)) > 1:
                    raise NotImplementedError("Can't handle multiple quote types / sizes yet (instrument {})".format(instrument))
                self._quote_handlers[(quote_type, quote_size, instrument.id)].append(on_quote)
                RecurringTask(lambda: self._call_quote_handlers(quote_type, quote_size, instrument.id), interval_sec=quote_size, init_sec=1, daemon=True)
    ...
    def _call_quote_handlers(self, quote_type, quote_size, ticker_id):
        """Generate a quote (of the given `quote_type` and `quote_size`) for `ticker_id` and call any registered quote handlers."""
        instrument = self._instruments.get(ticker_id)
        acc = self._ticumulators.get(ticker_id)
        handlers = self._quote_handlers.get((quote_type, quote_size, ticker_id))
        if acc is None or instrument is None or handlers is None:
            self.log.warning('No instrument, ticumulator, or handlers found for ID %d calling %s %f bar handlers', ticker_id, quote_type, quote_size)
        else:
            quote = acc.quote()
            for handler in handlers:
    ...
    def _error(self, msg):
        ...
        if code in DISCONNECT_ERRORS:
            self.log.error(msg.errorMsg + ' [{}]'.format(msg.errorCode))
            self.connected = False
        if not isinstance(code, int):
            self.log.error(str(msg))
        elif 2100 <= code < 2200:
            self.log.warning(msg.errorMsg + ' [{}]'.format(msg.errorCode))
        else:
            if code in ORDER_RELATED_ERRORS:         # TODO: Some of these are actually warnings (like 399, sometimes...?)...
                ...
                if order:
                    ...
                    self.log.error('ORDER ERR %d %s %s', code, order, msg.errorMsg)
            ...
            if code in TICKER_RELATED_ERRORS:
                ...
                if err_q is None:
                    self.log.warning('Got ticker error for unexpected request id {}: {} [{}]'.format(msg.id, msg.errorMsg, code))
                else:
    ...
    def _tickSize(self, msg):
        ...
        if msg.field == TickType.BID_SIZE:
            ...
            # if math.isnan(acc.tick()[TICK_FIELDS.index('volume')]):
            #    acc.add('volume', msg.size)
        elif msg.field == TickType.OPEN_INTEREST:
            acc.add('open_interest', msg.size)

        self._call_tick_handlers(msg.tickerId, acc.tick())
    ...
    def _openOrder(self, msg):
        ...
        if msg.orderState.m_warningText:
            self.log.warning('Order %d: %s', msg.orderId, msg.orderState.m_warningText)
...
class Ticumulator:
    """Accumulates ticks (bid/ask/last/volume changes) into bars (open/high/low/close) and quotes (bid/ask/vwap/volume).

    `bar()` will return OHLC data since the last `bar()` call (or creation), allowing you to make bars of any duration you like.

    `quote()` will return the most recent bid/ask and the volume and volume-weighted price since the last `quote()` call.

    Until a tick of each type has been added, the first results may contain ``NaN`` values and the volume may be
    off.

    `time` is Unix timestamp, `lasttime` is Unix time (sec since epoch) of last trade.
    For `add()` and `tick()`, `volume` is total cumulative volume for the day.  For US stocks, it is divided by 100.
    For `bar()`, volume is since last `bar()`.  For `quote()`, volume is since last `quote()`.
    """
    #: 'what' inputs to `add()` and outputs of `tick()`.
    TICK_FIELDS = ('time', 'bid', 'bidsize', 'ask', 'asksize', 'last', 'lastsize', 'lasttime', 'volume', 'open_interest')
    #: Outputs of `bar()`
    BAR_FIELDS = ('time', 'open', 'high', 'low', 'close', 'volume', 'open_interest')
    #: `quote()` fields
    QUOTE_FIELDS = ('time', 'bid', 'bidsize', 'ask', 'asksize', 'vwap', 'volume')
    ...
    def __init__(self):
        ...
        self.prev_bar_volume = float('NaN')
        # Computed for quote
        self.sum_last = 0.0
        self.sum_vol = 0.0
        self.prev_quote_volume = float('NaN')

    def add(self, what, value):
        """Update this Ticumulator with an input type `what` with the given float `value`.

        Valid `what` values are in `TICK_FIELDS` (except `time`).
        """
        self.time = time.time()
        if what not in self.TICK_FIELDS[1:]:
        ...
        if what in ('bid', 'ask', 'last'):
            ...
            self.close = value
        elif math.isnan(self.prev_bar_volume) and what == 'volume':       # Very first datapoint
            self.prev_bar_volume = self.prev_quote_volume = value

        # For QUOTE.  We arrange that lastsize comes in after the corresponding last price, since we get both from RTVOLUME.
        if what == 'lastsize':
            self.sum_last += self.last * self.lastsize      # self.lastsize == value, having been set above
            self.sum_vol += self.lastsize

    def tick(self):
        """:Return: the current quote snapshot: time, bid, bidsize, ask, asksize, last, lastsize, lasttime, volume, open_interest

        `time` is the local Unix timestamp of the last update to any field.
        """
        return self.time, self.bid, self.bidsize, self.ask, self.asksize, self.last, self.lastsize, self.lasttime, self.volume, self.open_interest

    def bar(self):
        """:Return: local unix timestamp of this call, open, high, low, close, volume (since last bar), open interest.

        Reset the accumulators for the next bar.  Volume is since the last bar.  `time` is effectively the close time."""
        bar = self.peek()
        self.open = self.close
        self.high = max(self.bid, self.ask, self.last)
        self.low = min(self.bid, self.ask, self.last)
        self.prev_bar_volume = self.volume
        return bar

    def quote(self):
        """:Return: ``(time, bid, bidsize, ask, asksize, vwap, volume)``.

        `bid`, `bidsize`, `ask`, `asksize` are most recent quotes; `vwap` is volume-weighted average price of trades since last call to `quote()`.
        `vwap` and `volume` will be 0 if there were no trades since the last call.
        `time` is local unix timestamp of this call.
        """
        vals = time.time(), self.bid, self.bidsize, self.ask, self.asksize, (self.sum_last / self.sum_vol) if self.sum_vol else 0.0, self.volume - self.prev_quote_volume
        self.prev_quote_volume = self.volume
    ...
    def peek(self):
        ...
        return time.time(), self.open, self.high, self.low, self.close, self.volume - self.prev_bar_volume, self.open_interest


#: Names of values in a tick tuple
TICK_FIELDS = Ticumulator.TICK_FIELDS
#: Convenience namedtuple for referencing tick fields
Tick = namedtuple('Tick', TICK_FIELDS)
#: Names of values in a bar tuple
BAR_FIELDS = Ticumulator.BAR_FIELDS
#: Convenience namedtuple for referencing bar fields
Bar = namedtuple('Bar', BAR_FIELDS)
#: Names of values in a quote tuple
QUOTE_FIELDS = Ticumulator.QUOTE_FIELDS
#: Convenince namedtuple for referencing quote fields
Quote = namedtuple('Quote', QUOTE_FIELDS)
...
def main():
    ...
    def on_tick(instrument, timestamp, bid, bidsize, ask, asksize, last, lastsize, lasttime, volume, open_interest):
    ...
    ib.register(inst, on_bar=None, on_tick=None, on_quote=on_quote, on_order=on_order, on_alert=on_alert, quote_size=1)     #   lambda *args: None
    try:
        for _ in range(20):
